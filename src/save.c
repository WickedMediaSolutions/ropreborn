...
bool load_char_obj (DESCRIPTOR_DATA * d, char *name)
{
    ...
    /* ROP Phase 1: Format Version Marker
     * When format_version is incremented to support new ROP fields, add version check:
     *
     * if (found && ch->version < 5)  // when adding ROP fields
     * {
     *     // Initialize new ROP fields for old character files
     *     ch->sect_number = 0;              // Unassigned sect
     *     ch->warpoint = 0;                 // No warpoints
     *     ch->profession_rank = 0;          // No profession rank
     *     ch->remort_count = 0;             // No remorts
     *     ch->remort_benefits_applied = 0;  // Remort benefits not applied
     *     // Remort bonuses initialized in new_char()
     * }
     *
     * ROP fields are read in fread_char() as individual KEY() blocks:
     *   - Look for "Sect", "Warpoint", "ProfessionRank", "RemortCount", etc.
     *   - Add their KEY() entries in fread_char() 'S', 'W', 'P', 'R' sections
     *
     * ROP fields are written in fwrite_char() with fprintf():
     *   - Need corresponding fwrite_char() updates (see save section)
     */
    return found;
}
        ...
        switch (UPPER (word[0]))
        {
           ...
           if( ( fMatch = imc_loadchar( ch, fp, word ) ) )
            ...
            case 'P':
                ...
                KEY ("PnkR", ch->profession_rank, fread_number (fp));
                break;

            case 'R':
                KEY ("Race", ch->race, race_lookup (fread_string (fp)));
                KEY ("Rmrt", ch->remort_count, fread_number (fp));
                ...
                if (!str_cmp (word, "RmBa"))
                {
                    ch->remort_hp_bonus = fread_number (fp);
                    ch->remort_move_bonus = fread_number (fp);
                    ch->remort_skill_slots = fread_number (fp);
                    ch->remort_no_food = fread_number (fp) ? TRUE : FALSE;
                    ch->remort_no_drink = fread_number (fp) ? TRUE : FALSE;
                    fMatch = TRUE;
                    break;
                }
            ...
            case 'S':
                ...
                KEY ("Sect", ch->sect_number, fread_number (fp));
