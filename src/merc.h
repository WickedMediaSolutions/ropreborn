...
#define CON_PRESS_ENTER			-9
#define CON_ANSI			-8
#define CON_GET_TELNETGA		-7
#define CON_GET_NEW_RACE		-6
#define CON_GET_NEW_SEX			-5
#define CON_GET_NEW_CLASS		-4
#define CON_GET_ALIGNMENT		-3
#define CON_DEFAULT_CHOICE		-2 
#define CON_GEN_GROUPS			-1 
#define CON_PICK_WEAPON			0
...
/*
 * Profession table structure (ROP)
 */
struct profession_type
{
    char *  name;                   /* e.g., "Warrior" */
    char    who_name[4];            /* e.g., "War" */
    sh_int  attr_prime;             /* primary stat: STR/INT/WIS/DEX   */
    sh_int  weapon_class;           /* preferred weapon type           */
    sh_int  skill_adept;            /* skill cap bonus % */
    sh_int  thac0;                  /* to-hit bonus */
    sh_int  hp_min;                 /* hp gains per level min */
    sh_int  hp_max;                 /* hp gains per level max */
    sh_int  mana_mult;              /* mana gain multiplier for casters */
    char *  base_group;             /* group name for base skills */
    char *  default_group;          /* group name for default skills */
};

/*
 * Sect table structure (ROP)
 */
struct sect_type
{
    char *  name;                   /* e.g., "Aethelhelm" */
    char    who_name[4];            /* e.g., "AET" */
    sh_int  alignment;              /* GOOD=1 or EVIL=-1 */
    char *  avatar_string;          /* e.g., "Avatar of Aethelhelm" */
    sh_int  hall_vnum;              /* sect house location vnum */
    char *  base_group;             /* group name for sect-exclusive spells */
    char *  description;            /* sect lore/description */
};
...
struct    char_data
{
    ...
    sh_int             trust;
    /* ROP fields */
    sh_int             sect_number;        /* sect alignment, 0-7 */
    int                warpoint;           /* PK economy currency */
    sh_int             profession_rank;    /* tracks progression for remort benefits */
    sh_int             remort_count;       /* number of remorts completed */
    bool               remort_benefits_applied; /* flag for stat persistence */
    /* Remort benefits customization */
    sh_int             remort_hp_bonus;    /* HP bonus applied on remort */
    sh_int             remort_move_bonus;  /* move bonus applied on remort */
...
extern    const    struct    profession_type profession_table [MAX_PROFESSION];
